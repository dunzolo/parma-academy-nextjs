...
export default function Page({ slug }: Props) {
  ...
  const handleFirstGroup = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const groups = await getRankingByGroup(["I", "J", "K"], slug as string);
    await generateFirstGroup(groups, "group_eso_primo_anno_first");
  };

  const handleSecondGroup = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const groups = await getRankingByGroup(["I", "J", "K"], slug as string);
    await generateSecondGroup(groups, "group_eso_primo_anno_second");
  };

  const handleThirdGroup = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const groups = await getRankingByGroup(["I", "J", "K"], slug as string);
    await generateThirdGroup(groups, "group_eso_primo_anno_third");
  };

  const handleFourthGroup = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const groups = await getRankingByGroup(["I", "J", "K"], slug as string);
    await generateFourthGroup(groups, "group_eso_primo_anno_fourth");
  };

  const generateFirstGroup = async (groups: SquadGroup[][], name: string) => {
    //recupero dal girone I la prima classificata
    const first_team = parseInt(groups[0][0].squad_id.id);
    //recupero dal girone J la terza classificata
    const second_team = parseInt(groups[1][2].squad_id.id);
    //recupero dal girone K la quarta classificata
  ...
  const generateSecondGroup = async (groups: SquadGroup[][], name: string) => {
    //recupero dal girone J la prima classificata
    const first_team = parseInt(groups[1][0].squad_id.id);
    //recupero dal girone K la terza classificata
    const second_team = parseInt(groups[2][2].squad_id.id);
    //recupero dal girone I la quarta classificata
  ...
  const generateThirdGroup = async (groups: SquadGroup[][], name: string) => {
    //recupero dal girone K la prima classificata
    const first_team = parseInt(groups[2][0].squad_id.id);
    //recupero dal girone I la terza classificata
    const second_team = parseInt(groups[0][2].squad_id.id);
    //recupero dal girone J la quarta classificata
  ...
  const generateFourthGroup = async (groups: SquadGroup[][], name: string) => {
    //recupero dal girone I la seconda classificata
    const first_team = parseInt(groups[0][1].squad_id.id);
    //recupero dal girone J la seconda classificata
    const second_team = parseInt(groups[1][1].squad_id.id);
    //recupero dal girone K la seconda classificata
  ...
  return (
    <div className="container">
      ...
      {selectedForm === "form1" && (
        <NewMatchForm slug={slug} group={"eso_primo_anno_first"} />
      )}
      {selectedForm === "form2" && (
        <NewMatchForm slug={slug} group={"eso_primo_anno_second"} />
      )}
      {selectedForm === "form3" && (
        <NewMatchForm slug={slug} group={"eso_primo_anno_third"} />
      )}
      {selectedForm === "form4" && (
        <NewMatchForm slug={slug} group={"eso_primo_anno_fourth"} />
      )}
    </div>
  );
}
